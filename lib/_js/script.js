...
socket.on('askJoin', function(data) {
	if(data.to == currentuser.user)
	{
		$("body").append("<div id='askJoin' style='position: absolute; top: 30%; left:30%; height: 40%; width:40%; background-color: green;'>User " + data.from + " ask to join your Party.<a href='javascript:accept(" + data.from + ");'>Accept!</a> <a href='javascript:deny(" + data.from + ");'>Deny !</div>")
		console.log("Anfrage to join von " + data.from + ".");
	}
});
function accept(data) {
	$("#askJoin").remove();
	socket.emit('acceptJoinRoom', data);
}
function deny(data) {
	$("#askJoin").remove();
	socket.emit('denyJoinRoom', data);
}
...
socket.on('onlineuser', function(data) {
	if(friendsonline == 0)
		$("#onlineusers").append("<span id='" + data.userid + "'><a href='javascript:askToJoin(" + data.userid + ");'>" + data.userid + "</a></span>");
	else
		$("#onlineusers").append("<span id='" + data.userid + "'><a href='javascript:askToJoin(" + data.userid + ");'>, " + data.userid + "</a></span>");
});
...
socket.on('user', function (data) {
	...
	{
		if(friendsonline == 0)
			$("#onlineusers").append("<span id='" + data.userid + "'>" + data.userid + "</span>");
		else
			$("#onlineusers").append("<span id='" + data.userid + "'>, " + data.userid + "</span>");
		audioJoin.play();
	}
...
function askToJoin(user) {
	console.log("User wird gefragt.");
	socket.emit('askJoinRoom', { userid: user });
}